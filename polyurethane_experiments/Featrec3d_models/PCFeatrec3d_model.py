...
class FeatureExtractors(torch.nn.Module):
    def __init__(self, device, group_size=128, num_group=1024):
        super().__init__()
        self.device = device

        # XYZ (3D) backbone only
        ...
        # Optionally limit the number of transformer blocks
        layers_keep = 12
        self.xyz_backbone.blocks.blocks = torch.nn.Sequential(*self.xyz_backbone.blocks.blocks[:layers_keep])
...
def fps(data, number):
    ...
    B, N, C = data.shape
    _, S = fps_idx.shape
    data_t = data.transpose(1, 2).contiguous()  # B x C x N
    fps_idx_expand = fps_idx.unsqueeze(1).expand(-1, C, -1)  # B x C x S
    fps_data = torch.gather(data_t, 2, fps_idx_expand).transpose(1, 2).contiguous()  # B x S x C
...
class Group(nn.Module):
 ...
 def forward(self, xyz):
    ...
    # Sample group centers
    center, center_idx = fps(xyz.contiguous(), self.num_group)  # B G 3

    # Get group indices via KNN
    idx = self.knn(xyz, center).permute(0, 2, 1)  # B G M

    ori_idx = idx.clone()

    idx_base = torch.arange(0, batch_size, device=xyz.device).view(-1, 1, 1) * num_points
    idx = idx + idx_base  # B G M
    idx = idx.reshape(batch_size * self.num_group * self.group_size)
